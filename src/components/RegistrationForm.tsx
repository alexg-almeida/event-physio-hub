...
import { Users, Calendar, MapPin, Clock, DollarSign, Gift } from "lucide-react";
...
const RegistrationForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const isEventoGratuito = evento.valor_inscricao === 0;
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          status_pagamento: isEventoGratuito ? 'pago' : 'pendente',
          data_pagamento: isEventoGratuito ? new Date().toISOString() : null,
          valor_pago: isEventoGratuito ? 0 : null,
        })
        .select()
        .single();
